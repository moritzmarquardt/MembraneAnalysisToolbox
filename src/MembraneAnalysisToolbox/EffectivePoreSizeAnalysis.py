...
class EffectivePoreSizeAnalysis:
    ...
    def __init__(
        ...
        if os.path.splitext(topology_file)[1] == ".tpr" and trajectory_file is None:
            raise ValueError(
                "A trajectory file is required when using a .tpr topology file."
            )
        if not (
            os.path.splitext(topology_file)[1] == ".tpr"
            or os.path.splitext(topology_file)[1] == ".gro"
        ):
        ...
        self.z_min, self.z_max = self._find_zConstraints(
            self.membrane_atom_positions
        )  # find the z constraints self.z_min and self.z_max
        # self.z_min = 133
        # self.z_max = 143
        self.y_middle, self.y_range = self._find_yConstraints(
            self.membrane_atom_positions
        )  # find the y constraints self.y_min and self.y_max

        self.membrane_atom_positions_filtered = self._filterPositions(
            self.membrane_atom_positions,
            self.y_middle,
            self.y_range,
            self.z_min,
        ...
        self.solvent_atom_positions_filtered = self._filterPositions(
            self.solvent_atom_positions,
            self.y_middle,
            self.y_range,
            self.z_min,
            self.z_max,
        ...
        if self.verbose:
            print(
                "Positions filtered after " + str(time.time() - self.start) + " seconds"
            )

        bins = "auto"
        self.membrane_hist, self.membrane_hist_edges = np.histogram(
            self.membrane_atom_positions_filtered, density=1, bins=bins
        )
        self.solvent_hist, self.solvent_hist_edges = np.histogram(
            self.solvent_atom_positions_filtered, density=1, bins=bins
        )
        if self.verbose:
            print(
                "Histograms calculated after "
                + str(time.time() - self.start)
                + " seconds"
            )

        def membrane_hist_func(x):
            membrane_hist_edges_middle = (
                self.membrane_hist_edges[0:-1] + np.diff(self.membrane_hist_edges) / 2
            )
            front_edge = (
                membrane_hist_edges_middle[0] * 2 - membrane_hist_edges_middle[1]
            )
            back_edge = (
                membrane_hist_edges_middle[-1] * 2 - membrane_hist_edges_middle[-2]
            )
            membrane_hist_edges_middle = np.append(
                front_edge, np.append(membrane_hist_edges_middle, back_edge)
            )
        ...
        def solvent_hist_func(x):
            solvent_hist_edges_middle = (
                self.solvent_hist_edges[0:-1] + np.diff(self.solvent_hist_edges) / 2
            )
            front_edge = solvent_hist_edges_middle[0] * 2 - solvent_hist_edges_middle[1]
            back_edge = (
                solvent_hist_edges_middle[-1] * 2 - solvent_hist_edges_middle[-2]
            )
            solvent_hist_edges_middle = np.append(
                front_edge, np.append(solvent_hist_edges_middle, back_edge)
            )
        ...
        if self.verbose:
            print(
                "Histogram functions defined after "
                + str(time.time() - self.start)
                + " seconds"
            )
    ...
    def analyseConstraints(self):
        ...
        plt.legend()
        plt.axvline(
            x=self.y_middle - self.y_range / 2, color="r", linestyle="--"
        )  # y_min line
        plt.axvline(
            x=self.y_middle + self.y_range / 2, color="r", linestyle="--"
        )  # y_max line
    ...
    def _filterPositions(self, positions, y_middle, y_range, z_min, z_max):
        ...
        filtered_indices = np.where(
            (positions[:, :, 1] >= y_middle - y_range / 2)
            & (positions[:, :, 1] <= y_middle + y_range / 2)
            & (positions[:, :, 2] >= z_min)
            & (positions[:, :, 2] <= z_max)
        )
        filtered_x_positions = positions[
            filtered_indices[0], filtered_indices[1], 0
        ].flatten()
    ...
    def _calculate_pore_edges(self, strategy="averaging"):
        ...
        if strategy == "averaging":
            ...
            first_zero_middle = (
                self.membrane_hist_edges[first_zero_bin]
                + self.membrane_hist_edges[first_zero_bin + 1]
            ) / 2
            first_non_zero_bin = np.where(self.solvent_hist > 0)[0][0]
            first_non_zero_middle = (
                self.solvent_hist_edges[first_non_zero_bin]
                + self.solvent_hist_edges[first_non_zero_bin + 1]
            ) / 2
            avrg_lower_edge = np.abs(first_zero_middle + first_non_zero_middle) / 2

            last_zero_bin = np.where(self.membrane_hist == 0)[0][-1]
            last_zero_middle = (
                self.membrane_hist_edges[last_zero_bin]
                + self.membrane_hist_edges[last_zero_bin + 1]
            ) / 2
            last_non_zero_bin = np.where(self.solvent_hist > 0)[0][-1]
            last_non_zero_middle = (
                self.solvent_hist_edges[last_non_zero_bin]
                + self.solvent_hist_edges[last_non_zero_bin + 1]
            ) / 2
    ...
    def analyseDensity(self):
        combined_positions = np.concatenate(
            (self.solvent_atom_positions, self.membrane_atom_positions), axis=1
        )
        ...
        filtered_positions = combined_positions[
            filtered_combined[0], filtered_combined[1], 0:2
        ]
    ...
    def analyseDensityNormalised(
        self, slice_height=10, factors=["scott", "scott"], skip=50
    ):
        ...
        filtered_indices_mem = np.where(
            ...
            & (
                membrane_pos[:, :, 2]
                <= (self.z_min + self.z_max) / 2 + slice_height / 2
            )
        )
        ...
        filtered_positions_mem = membrane_pos[
            filtered_indices_mem[0], filtered_indices_mem[1], 0:2
        ]
        filtered_positions_sol = solvent_pos[
            filtered_indices_sol[0], filtered_indices_sol[1], 0:2
        ]
        ...
        plt.title(
            "Density of C and "
            + " & ".join(map(str, self.solvent_resnames))
            + " molecules normed",
            fontsize="x-large",
        )
        plt.xlabel("X-axis in nm", fontsize="x-large")
